...
FIRST_EMOJI = Emojis.fastforward_left   # [:track_previous:]
LEFT_EMOJI = Emojis.forward_left    # [:arrow_left:]
RIGHT_EMOJI = Emojis.fastforward_right   # [:arrow_right:]
LAST_EMOJI = Emojis.forward_right    # [:track_next:]
DELETE_EMOJI = Emojis().trash
    ...
    * max_lines: `int`
    ...
    def add_line(self, line: str = '', *, empty: bool = False) -> None:
        ...
        if len(line) > (max_chars := self.max_size - len(self.prefix) - 2):
            if len(line) > self.scale_to_size:
                line, remaining_words = self._split_remaining_words(
                    line, max_chars)
                if len(line) > self.scale_to_size:
                    log.debug(
                        "Could not continue to next page, truncating line.")
    ...
    def _split_remaining_words(self, line: str, max_chars: int) -> t.Tuple[str, t.Optional[str]]:
        ...
        return (
            ...
            continuation_header +
            " ".join(remaining_words) if remaining_words else None
        )
    ...
    async def paginate(
        ...
        if not lines:
            if exception_on_empty_embed:
                log.error("Pagination asked for empty lines iterable")
        ...
        for line in lines:
            try:
                ...
                log.error(f"Failed to add line to paginator: '{line}'")
                raise  # Should propagate
            else:
                log.info(f"Added line to paginator: '{line}'")
        ...
        if len(paginator.pages) <= 1:
            if footer_text:
                ...
                log.info(f"Setting embed footer to '{footer_text}'")

            if url:
                embed.url = url
                log.info(f"Setting embed url to '{url}'")
            ...
            log.info(f"Setting embed footer to '{embed.footer.text}'")
