...
class AutoConfig(commands.Cog):
    ...
    def guild_id(self) -> int:
        ...
        if not self.bot.get_guild(int(os.getenv('Guild_ID'))):
            logger.info(
                f"Cannot find the guild with id {os.getenv('Guild_ID')}. Rasing error!")
            
            raise NotThere(f"Cannot find guild with ids {os.getenv('Guild_ID')}")
    ...
    def check(self) -> t.Union[bool, dict]:
        ...
        roles = [r for r in self.required()['Roles'].keys()
                 if not discord.utils.get(guild.roles, name=r)]

        #checking for the required channels
        channels = [c for c in self.required()['Roles'].keys()
                    if not discord.utils.get(guild.channels, name=c)]

        if not channels and not roles:
            logger.info(
                'The check passed, the guild has all the roles and channels.!')
            return False
            ...
            logger.info(
                'The guild does not have either the roles nor the channels.')
    ...
    async def _create(self) -> None:
        ...
        if not self.check():
            ...
            logger.info(
                'The server already has the required roles and channels, getting the ids!')
            ...
            try:
                for role in self.check()['Roles']:
                    await guild._createrole(name=role)

            except Forbidden:
                logger.warning(
                    'The bot does not have required permissions to create the roles!')
                return
            ...
            try:
                for channel in self.required()['Channels']:
                    await guild._createtext_channel(name=channel)
            except Forbidden:
                logger.warning(
                    'The bot does not have required permissions to create the channels!')
                return

            else:
                logger.info('Successfully created all the channels.')

        else:
            logger.info(
                'Creating the required roles and the channels for the server.')
        ...
        channel = discord.utils.get(
            guild.text_channels, name=random.choice(guild.text_channels).name)
        channel_invite = await channel._createinvite()
    ...
    async def settingup(self, ctx: Context) -> None:
        ...
        if self.auto:
            raise RuntimeError(
                "Auto-config is set to true! Cannot use setup command.")
